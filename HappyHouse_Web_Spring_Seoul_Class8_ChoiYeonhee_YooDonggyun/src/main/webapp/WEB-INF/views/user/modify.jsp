...
<body>
   ...
   <%-- <c:if test="${id == null}">
      <form method="post" action="${pageContext.request.contextPath}/user/modify.do">
           <h1>수정할 회원의 정보를 입력하세요</h1>
...
<c:if test="${id == null}">
           <h2>수정할 회원의 정보를 입력하세요</h2>
	<form id="join_form" name="join_form" method="post">
	<label for="mbr_id"><strong>ID  <span id="id_chk_res"></span></strong></label>
	<input type="text" id="mbr_id" name="id" value="${userId}" class="form-control mb-3" placeholder="ID" maxlength="20" required="required" />

	<label for="mbr_pwd"><strong>Password<span id="pwd_chk_res" class="text-danger"></span></strong></label>
	<input type="text" id="mbr_pwd" name="password" class="form-control mb-3" placeholder="Password" maxlength="20" required="required" />
	...
	<input type="submit" class="btn btn-primary btn-block" value="수정" maxlength="50"
               formaction="${pageContext.request.contextPath}/user/modify.do"/>
   ...
   <c:if test="${id != null}">
  	 <div class="modal-header">
		<h4 class="modal-title">회원정보 수정 완료</h4>
	</div>
	
      <a style="margin:0 auto" href="${pageContext.request.contextPath}/user/logout.do">메인화면</a>
   </c:if>
   
   
  <%--  <c:if test="${id != null}">
      <h1>회원정보 수정 완료</h1>
      <a href="${pageContext.request.contextPath}/user/logout.do">메인화면</a>
   </c:if> --%>
   <!-- here end -->
